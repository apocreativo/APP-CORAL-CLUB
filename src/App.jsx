...
function makeGrid(count = 20) {
  ...
  return Array.from({ length: count }).map((_, i) => {
    ...
    return { id: i + 1, state: "av", price: 0, x: +x.toFixed(4), y: +y.toFixed(4) };
  });
}
...
export default function App(){
  ...
  // Auto-seed tents if empty and load from localStorage
  useEffect(()=>{
    try{
      const saved = localStorage.getItem("coralclub:localState");
      if(saved){
        const parsed = JSON.parse(saved);
        setData(d => ({ ...d, ...parsed, tents: (parsed.tents && parsed.tents.length)? parsed.tents : (d.tents && d.tents.length? d.tents : makeGrid(d.layout?.count||20)) }));
      }else{
        setData(d => ({ ...d, tents: (d.tents && d.tents.length)? d.tents : makeGrid(d.layout?.count||20) }));
      }
    }catch(_e){ /* ignore */ }
  }, []);

  // Persist minimal state locally
  useEffect(()=>{
    try{
      const minimal = { tents: data.tents, reservations: data.reservations, payments: data.payments };
      localStorage.setItem("coralclub:localState", JSON.stringify(minimal));
    }catch(_e){ /* ignore */ }
  }, [data.tents, data.reservations, data.payments]);
// UI
  ...
  // Expire pending holds after timeout
  useEffect(()=>{
    const id = setInterval(()=>{
      const now = new Date().toISOString();
      const expired = (data.reservations||[]).filter(r=> r.status==="pending" && r.expiresAt && r.expiresAt < now);
      if(expired.length){
        let tentsUpd = data.tents;
        let resUpd = data.reservations;
        for(const r of expired){
          tentsUpd = tentsUpd.map(t=> t.id===r.tentId ? { ...t, state:"av" } : t);
          resUpd = resUpd.map(x=> x.id===r.id ? { ...x, status:"expired" } : x);
        }
        mergeState({ tents: tentsUpd, reservations: resUpd }, "Expirar holds");
      }
    }, 15000);
    return ()=> clearInterval(id);
  }, [data.tents, data.reservations]);
const [myPendingResId, setMyPendingResId] = useState(null);
  ...
  const mergeState = async (patch, logMsg) => {
    try{
      ...
      // Fallback local: apply patch in-memory and bump a local rev
      setData(s => ({ ...s, ...patch }));
      setRev(r=> (r||0)+1);
  ...
  return (
    <div className="app-shell" onMouseMove={onMouseMove} onMouseUp={onMouseUp}>
      <div className="phone">
        ...
        {/* Floating reservation countdown badge */}
        :{String(ss).padStart(2,'0')}</span>
          </div>
        )}
                  ...
                  <div>
                    ...
                    <div className="item">
                      ...
                      <div className="row" style={{ gap:8, flexWrap:"wrap", alignItems:"center" }}>
                        ...
                        <div className="hint">Este PIN es el que pide la ventana de acceso a Admin.</div>
                      </div>
                    </div>
